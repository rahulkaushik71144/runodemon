...
- **Find queries**: If the query is about retrieving data, return a `find` query as a JSON object.
- **Aggregation queries**: If the query requires grouping, sorting, or calculations, return an aggregation pipeline as a JSON array.
- **Insert queries**: If the query asks to add a document, return an `insertOne` or `insertMany` query as a JSON object.
- **Update queries**: If the query requires modifying data, return an `updateOne` or `updateMany` query as a JSON object.
- **Delete queries**: If the query requires removing data, return a `deleteOne` or `deleteMany` query as a JSON object.
- **Count queries**: If the query asks "how many" or "count", return a `countDocuments` query.
